    ...
    if (self.frame - self.last_button_frame) * DT_CTRL > 0.2:
      ...
      if button_counter_offset is not None:
        if self.CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
          ...
          msg_fn = hyundaicanfd.create_buttons_alt
        else:
          counter = (CS.buttons_counter + button_counter_offset) % 0xF
          msg_fn = hyundaicanfd.create_buttons
        for _ in range(20):
          can_sends.append(msg_fn(packer, self.CP, CAN, counter, send_button))
        self.last_button_frame = self.frame
