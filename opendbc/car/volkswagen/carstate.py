...
class CarState(CarStateBase):
  ...
  def update_pq(self, pt_cp, cam_cp, alt_cp, ext_cp) -> structs.CarState:
    ret = structs.CarState()
  ...
  def update_meb(self, pt_cp, cam_cp, alt_cp, ext_cp) -> structs.CarState:
    ret = structs.CarState()
    ...
    self.steering_curvature = -pt_cp.vl["QFK_01"]["Curvature"] * (1, -1)[int(pt_cp.vl["QFK_01"]["Curvature_VZ"])]
    ...
    self.radar_disable_failed = True if RADAR_DISABLE_STATE["error"] == True and self.CP.flags & VolkswagenFlags.DISABLE_RADAR else False
    ...
    self.speed_limit = self.speed_limit_mgr.get_speed_limit()
    self.speed_limit_predicative = self.speed_limit_mgr.get_speed_limit_predicative()
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      ...
      # EV battery details
      self.battery_charge = pt_cp.vl["Motor_16"]["MO_Energieinhalt_BMS"] # battery charge WattHours
      if self.CP.networkLocation == NetworkLocation.gateway:
        self.battery_heater_active = bool(alt_cp.vl["MEB_HVEM_03"]["PTC_ON"]) # battery heater active
        self.battery_voltage      = alt_cp.vl["MEB_HVEM_01"]["Battery_Voltage"] # battery voltage
        self.battery_capacity     = alt_cp.vl["BMS_04"]["BMS_Kapazitaet_02"] * self.battery_voltage # EV battery capacity WattHours
        self.battery_soc          = self.battery_charge / self.battery_capacity * 100 if self.battery_capacity > 0 else 0 # battery SoC in percent
        self.battery_power        = alt_cp.vl["MEB_HVEM_01"]["Engine_Power"] # engine power output
        self.battery_temperature  = alt_cp.vl["DCDC_03"]["DC_Temperatur"] # dcdc converter temperature
